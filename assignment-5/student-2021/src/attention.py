...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        B, T, C = x.size() #NOTE: 这里的B、T、C分别代表什么? batch_size, sequence_length (block_size), total_dimension
...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        syn_kq= self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)

		# SynthesizerAttention; Synthesizer-attend: (B, nh, T, hs) x (hs, T) -> (B, nh, T, T); 
        #                       Calculate_weighted sum: (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs).
        syn_kq = F.relu(syn_kq) # (B, nh, T, hs)
        #att = att @ self.w2[None, None, ..., :T] + self.b2[..., :T]  # (B, nh, T, hs) -> (B, nh, T, T-1)
        att = syn_kq @ self.w2[:, :T] + self.b2[:T]	# (B, nh, T, hs) x (hs, T) + (T) -> (B, nh, T, T)
