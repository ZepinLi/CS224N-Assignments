...
class NameDataset(Dataset):
    """以下为finetune的数据组织方式(trick)，将要预测的地名用'??'給框出来。
    """
    ...
    def __getitem__(self, idx):
        ...
        x = x + self.PAD_CHAR*(self.block_size - len(x)) #NOTE：准备完毕input
        y = self.PAD_CHAR*(len(inp)-1) + x[len(inp):] #NOTE：准备完毕prediction
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_content_len = int(np.clip(masked_content_len, a_min=1, a_max=truncated_len - 2))  # Clip invalid random indices.
        masked_content_idx = np.random.randint(1, truncated_len - masked_content_len)
        #以下将doc拆成三部分 -- prefix、masked_content、suffix
