...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    centerWordVec = centerWordVec[:, np.newaxis]  # (d,) -> (d, 1) #其中d为隐藏层维度.
    y_hat = softmax(np.matmul(outsideVectors, centerWordVec).T).T  # (V, d) * (d,1) --> (V, 1)
    loss = -np.log(y_hat[outsideWordIdx])  # Negative-log likelihood, (V, 1) #此处为（a）的结论.
    # Construct y here.
    y = np.zeros_like(y_hat)  # (V, 1)
    # Or define: delta = y_hat[outsideWordIdx] - 1, and use delta instead.
    y[outsideWordIdx] = 1  # One-hot column vector. #这里y就是groundtruth.
    gradCenterVec = np.matmul(outsideVectors.T, y_hat - y)  # (d, V) * (V, 1) = (d, 1) #此处为（b）的结论.
    gradOutsideVecs = np.matmul(y_hat - y, centerWordVec.T)  # (V, 1) * (1, d) = (V, d). Result transposed. #此处为（c）的结论.
    ### END YOUR CODE
...
def getNegativeSamples(outsideWordIdx, dataset, K):
    ...
    for k in range(K):
        ...
        while newidx == outsideWordIdx: #NOTE：这里为什么要相等？这里是针对outsideWordIdx处做采样（即，负采样）；如果相等，表明找到了对应的负采样位置。
            newidx = dataset.sampleTokenIdx()
...
def negSamplingLossAndGradient(
    ...
    indices = [outsideWordIdx] + negSampleWordIndices #注意这里包含了当前的u_o，故维度为(K+1,d).
    ...
    sampleVec = -outsideVectors[indices] #NOTE：这里为什么要加上负号？对应于（g）中关于负采样中的simoid内部计算.
    sampleVec[0] *= -1  # (K+1, d)
    temp = sigmoid(np.matmul(sampleVec, centerWordVec)) - 1  # (K+1, d) * (d, 1) -> (K+1, 1)
    loss = -np.sum(np.log(temp + 1))  # This is a scalar!
    gradCenterVec = np.matmul(sampleVec.T, temp)  # (d, K+1) * (K+1, 1) -> (d, 1) #（g）-（1）结果.
    ...
    grad = -np.matmul(temp, centerWordVec.T) #（g）-（3）结果.
    grad[0] *= -1 #此为对u_o的偏导，由（g）-（2）得到的结果.
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWordInd in outsideWordIndices:
        ...
        gradOutsideVectors += wordGradOutsizeVecs  # They are of the same shape (V, d). #留意，这俩更新梯度的方式是累加.
        #这俩也正是SkipGram学习的参数.
